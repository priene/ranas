...
class Home extends CI_Controller {
	...
	function definirturnos(){
		...
		$datos["titulo"]="Ingresar banda";
		$this->load->view("templates/frontend/header2.php",$datos);
		$this->load->view("frontend/definirturnos.php",$datos);
		$this->load->view("templates/frontend/footer2.html");	
	}

	function definirconvocatorias(){
		$datos["valores"] = $this->info_model->get_bandassinconvocatoria();
		$datos["titulo"]="Ingresar banda";
		$this->load->view("templates/frontend/header2.php",$datos);
		$this->load->view("frontend/definirconvocatorias.php",$datos);
		$this->load->view("templates/frontend/footer2.html");
	}

	function validar_confirmarbandas(){
		if($this->session->userdata('logueado')){
			
			$bac = $this->input->post('bac');
			$resultado = $this->info_model->confirmarbandas($bac);
			if ($resultado)
			{
				$response["mensaje"] = "Los registro se han modificado correctamente.";
			}
			else
			{			
				$response["mensaje"] = "Hubo un problema al momento de modificar los registros. Intentelo nuevamente mas tarde.";
			}
			header('Content-type: application/json; charset=utf-8');
			echo json_encode($response);
			exit();
	...
	function validar_definirturnos(){
		if($this->session->userdata('logueado')){
			
			$bac = $this->input->post('bac');
			$defturno = $this->input->post('val');
			$resultado = $this->info_model->definirturnos($bac,$defturno);
			if ($resultado)
			{
				$response["mensaje"] = "Los registro se han modificado correctamente.";
			}
	...
	function validar_definirconvocatorias(){
		if($this->session->userdata('logueado')){
			
			$bac = $this->input->post('bac');
			$defturno = $this->input->post('val');
			$resultado = $this->info_model->definirconvocatorias($bac,$defconvocatoria);
			if ($resultado)
			{
				$response["mensaje"] = "Los registro se han modificado correctamente.";
			}
